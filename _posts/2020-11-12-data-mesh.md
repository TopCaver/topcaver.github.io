...
将领域数据集作为产品，需要增加新的技能：（a）数据产品负责人（b）数据工程师。
...
将数据基础架构构建为平台的关键是（a）不包含任何特定于域的概念或业务逻辑，使其保持领域无关性；以及（b）确保平台隐藏了所有潜在的复杂性，并在提供自助服务组件。自助数据基础架构平台可以向用户（域的数据工程师）提供诸多功能。这里列举几个：
